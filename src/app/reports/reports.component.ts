...
export class ReportsComponent implements OnInit,AfterViewInit  {
  ...
  @ViewChild('canvasBarangay',{static: false}) theCanvas : ElementRef;
  theContext : any;
  chartBarangay = [];
  check = false;
  ...
  getOWList = () => {
    this.reportService.getOverweight().subscribe(res => {
      this.overWeightList = res;
  ...
  loadAgeData(){
    ...
    console.log("The mode is :" +this.ToggleMode + "From Age Factor");
    for(let tempdata of this.overWeightList){
      if(tempdata.healthStatus == this.ToggleMode){
        console.log("The mode is :" + this.ToggleMode + " The Barangay is :" + tempdata.barangay);
    ...
    this.generatePatientsReport([this.maxChecker(BarangkaDrive), this.maxChecker(BarangkaIbaba),
      this.maxChecker(BarangkaIlaya) , this.maxChecker(BarangkaItaas), this.maxChecker(BuayangBato),
       this.maxChecker(Hulo), this.maxChecker(MabiniJRizal), 
  ...
  maxChecker(theValue) : number{
    console.log(theValue + "From the Age");
    if(theValue >6){
      return 6;
    }
    return theValue;
  ...
  onToggle() {
    console.log("Toggled");
    if (!this.check) {
      this.ToggleMode = "Under Weight";
      this.check = true;
      this.getUWList();
    } else {
      this.ToggleMode = "Over Weight";
      this.check = false;
      this.getOWList();
    }
  ...
  byVolume(){
    if(this.ToggleMode = "Under Weight"){
      this.getUWList();
    }else{
      this.getOWList();
    }
